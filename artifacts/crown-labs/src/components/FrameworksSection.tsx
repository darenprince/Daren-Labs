          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {frameworks.map((fw, i) => (
              <div key={i} className="rounded-xl border border-border bg-card p-6 space-y-4">
                <div>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${statusBadge(fw.status)}`}>
                    {fw.status}
                  </span>
                </div>
            ...
            {books.map((book, i) => (
              <div key={i} className="rounded-xl border border-border bg-card p-6 space-y-4">
                <div className="flex items-start justify-between">
                  ...
                  <span className={`px-2 py-0.5 rounded text-[10px] font-semibold border ${statusBadge(book.status)}`}>
                    {book.status}
                  </span>
                </div>
