import { BookOpen, ArrowRight, FileText, ShieldCheck, Layers3 } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-10">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm text-muted-foreground leading-relaxed">
              The landing experience and documentation describe the same portfolio, use the same readiness language,
              and point back to the same source of truth.
            </p>
          </div>
          <a href="/docs/CROWN_LABS_PORTFOLIO.md" className="inline-flex items-center gap-2 text-sm font-semibold text-foreground hover:text-primary transition-colors">
            Open portfolio record <ArrowRight className="h-4 w-4" />
          </a>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card overflow-hidden">
          <div className="flex items-center gap-3 px-5 py-4 border-b border-border">
            ...
            <div>
              <p className="text-sm font-semibold text-foreground">Documented product index</p>
              <p className="text-xs text-muted-foreground">{products.length} canonical portfolio records</p>
            </div>
          </div>
          <div className="divide-y divide-border">
            {products.map((product) => (
              <div key={product.id} className="grid gap-3 md:grid-cols-[minmax(0,1fr)_auto] px-5 py-4">
                <div>
                ...
                <div className="text-left md:text-right">
                  <p className="text-xs font-semibold text-foreground">{product.status}</p>
                  <p className="mt-1 text-[11px] text-muted-foreground">Next: {product.nextGate}</p>
                </div>
              </div>
            ))}
          </div>
