      ...
      <DialogContent className="sm:max-w-[600px] border-border bg-card p-0 overflow-hidden max-h-[90vh] flex flex-col">
        <div className="p-6 overflow-y-auto">
          <div className="flex flex-wrap gap-2 mb-4">
            <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold border uppercase tracking-widest ${getStatusColor(product.status)}`}>
              {product.status}
            </span>
            <span className={`px-2.5 py-0.5 rounded text-[10px] font-bold border uppercase tracking-widest ${getCategoryColor(product.category)}`}>
              {product.category}
            </span>
          </div>
          ...
          <div className="grid sm:grid-cols-2 gap-4 mb-6">
            <div className="rounded-lg border border-border bg-background/40 p-4">
              ...
              <div className="space-y-1">
                ...
                <div className="flex justify-between items-end">
                  ...
                  <span className="text-sm font-bold text-foreground">{product.valuationProjected}</span>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {product.metrics.map((metric, i) => (
                <div key={i} className="rounded-lg border border-border bg-background/40 p-3 flex flex-col justify-center items-center text-center">
          ...
          <div className="mb-6">
            <p className="text-xs font-semibold uppercase tracking-widest text-muted-foreground mb-3">Core Capabilities</p>
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <div className="w-full sm:w-auto">
                  <Button variant="outline" className="w-full sm:w-auto border-border text-muted-foreground pointer-events-none opacity-50">
                    View full page
                  </Button>
                </div>
              </TooltipTrigger>
