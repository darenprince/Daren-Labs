        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-4">
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="ra-name" name="name" placeholder="John Doe" className="border-border bg-background" />
              {errors.name && <p className="text-xs text-destructive">{errors.name}</p>}
            </div>
            <div className="space-y-2">
              <Label htmlFor="ra-email">Email address *</Label>
              <Input id="ra-email" name="email" type="email" placeholder="john@example.com" className="border-border bg-background" />
              {errors.email && <p className="text-xs text-destructive">{errors.email}</p>}
            </div>
            <div className="space-y-2">
              <Label htmlFor="ra-org">Organization (optional)</Label>
              <Input id="ra-org" name="org" placeholder="Company Inc." className="border-border bg-background" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="ra-usecase">Role / Use case</Label>
              <Textarea id="ra-usecase" name="usecase" placeholder="How do you plan to use this product?" className="border-border bg-background min-h-[80px]" />
            </div>
            <div className="flex items-start space-x-2 pt-2">
              <Checkbox
                id="ra-understand"
                checked={understood}
                onCheckedChange={(v) => { setUnderstood(!!v); setErrors(prev => ({ ...prev, understand: "" })); }}
                className="mt-1"
              />
              <div className="grid gap-1.5 leading-none">
                <Label htmlFor="ra-understand" className="text-sm font-normal cursor-pointer">I understand this is a pre-release product</Label>
                {errors.understand && <p className="text-xs text-destructive">{errors.understand}</p>}
              </div>
            </div>
          </div>
