...
export function BetaTestersModal({ trigger }: { trigger: React.ReactNode }) {
  ...
  const products = [
    ...
    "Crown Forensics Lab", "Crown Relations AI",
  ];
        ...
        <form onSubmit={handleSubmit} className="space-y-6 pt-4">
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="bt-name" name="name" placeholder="John Doe" className="border-border bg-background" />
              {errors.name && <p className="text-xs text-destructive">{errors.name}</p>}
            </div>
            <div className="space-y-2">
              <Label htmlFor="bt-email">Email address *</Label>
              <Input id="bt-email" name="email" type="email" placeholder="john@example.com" className="border-border bg-background" />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {products.map(p => (
                  <div key={p} className="flex items-center space-x-2">
                    ...
                    />
                    <Label htmlFor={`bt-product-${p}`} className="text-xs font-normal leading-tight cursor-pointer">{p}</Label>
                  </div>
                ))}
              </div>
            </div>
            <div className="space-y-2">
              ...
              <Textarea id="bt-expertise" name="expertise" placeholder="Tell us about your technical or industry background..." className="border-border bg-background min-h-[80px]" />
            </div>
          </div>
          <Button type="submit" disabled={loading} className="w-full bg-primary text-primary-foreground hover:bg-primary/90">
            {loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Submitting...</> : "Join beta program"}
          </Button>
        </form>
      </DialogContent>
    </Dialog>
